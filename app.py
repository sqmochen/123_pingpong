...
def init_db():
    ...
    with get_conn() as conn:
        ...
        for username, pw, role, dname in accounts:
            cur.execute(
        ...
        if len(coach_users) >= 2:
            ...
            for uid, name, phone, bio, spec in coach_details:
                cur.execute(
                    "INSERT OR IGNORE INTO Coaches(user_id,name,phone,bio,specialty) "
                    "VALUES(?,?,?,?,?)",
        ...
        if students and courses:
            for sid, cid in [
                ...
                cur.execute(
                    "INSERT OR IGNORE INTO Enrollments(student_id,course_id,enrolled_date)"
            ...
            for sid, cid, amt in [
                ...
                if not exists:
                    cur.execute(
                        ...
                        " VALUES(?,?,?,?,?)", (sid, cid, amt, period, now_str))
...
def login_page():
    ...
    with col_c:
        ...
        st.markdown("---")
        st.caption(
            "預設測試帳號：admin / admin123 ｜ coach01 / coach123 ｜ student01 / stu123")
                ...
                for nm in already:
                    ...
                    if c2.button("移除", key=f"rm_{smap[nm]}_{cid_enroll}"):
                        with get_conn() as conn:
                            ...
                            conn.execute("""
                                UPDATE LeaveRequests
                                SET status='rejected', reject_reason='已退課'
